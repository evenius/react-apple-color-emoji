import { writeFile, stat, readFile, readdir, rename } from 'fs/promises';
...
async function writeData() {
  ...
  const emojiFetchers = emojiList.map(({ emoji, codePoints, name }) => async () => {
    ...
    const aliasEmojiName = `export const em${canonicalName} = ${makeDefinition(canonicalName, aliases, fileName.replace('png', 'webp'), "u" + utf16.join('_'), 160, 160)};`
    const emojiDefinitionFile = emojiDefinitionFileTemplate(canonicalName, aliases, fileName.replace('png', 'webp'), "u" + utf16.join('_'), 160, 160);
  ...
  await Promise.all(definitionInfo.map(
    ([normalName, definitionFile]) => writeFile(
        resolve(__dirname, `../src/icon/em${normalName}.ts`), 
        definitionFile.replace(/PATH:::(.*?):::/g, "import($1)")
    )
  ));
...
base64EncodeEmojisTS(resolve(__dirname, '../src/emojis.ts'))
...
function makeDefinition(normalisedName: string, aliases: string[], fileName: string, unicode: string, width: number, height: number) {
  return `{
  icon: PATH:::"../webp/${fileName}":::,
  name: "${normalisedName}",
...
const definition = ${makeDefinition(normalisedName, aliases, fileName, unicode, width, height)};
...
async function encodeWebpToBase64(filePath: string) {
  const data = await readFile(filePath);
  return Buffer.from(data).toString('base64');
}

const base64EncodeEmojisTS = async (inputPath: string, outputPath?: string) => {
  ...
  let useTmp = !outputPath || inputPath === outputPath;
  outputPath = useTmp ? inputPath.replace('.ts', '.tmp.ts') : outputPath!;

  const baseDir = resolve(__dirname, '../src');
  const output = createWriteStream(outputPath);
  
  for await (const line of readInterface) {
    let matches = line.match(/PATH:::\"(.*?)\":::/);

      if(matches && matches.index) {
        let webpPath = matches[1];
        const base64 = await encodeWebpToBase64(resolve(baseDir, webpPath));

        let modifiedLine = line.slice(0, matches.index) + `"data:image/webp;base64,${base64}"` + line.slice(matches.index + matches[0].length);
